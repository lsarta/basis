...
1. **Product type is fixed to MULTIFAMILY for this build.** Every scraped deal is NYC multifamily. This controls away product-type heterogeneity so the regime axis is clean. Multifamily chosen because: highest ACRIS transaction volume (the scrape actually fills cohorts), CEMAs concentrate here (the hero beat and the product type agree), and the gold-set deals (incl. 300 W 44th) are already in this regime.
   - **AP/CR honesty (found by the scrape's grounding audit):** the v1 cohort population filters to ACRIS `property_type='AP'` (institutional apartment buildings — excludes sub-10-unit coops/walk-ups by design) for product-type purity. But large MF towers often code **`CR`** (commercial real estate): the **300 W 44th hero and 1905 Third record as CR**. So CR-coded MF towers — including the hero — are demoed via the **preselected "decompose this deal" lookup, not the cohort sweep**. Widening to `CR`-with-a-residential-guard is the roadmap toward all-CRE. The hero is not in the swept cohort population, and we say so rather than hide it.
...
- **Two distinct gates, both true and non-contradictory:**
  ...
  - **(b) v1 scrape gold-replay = hard-gates on the 12 rows it attempts.** The v1 ACRIS scrape extracts deeds, CEMA recoveries (incl. 300 W 44th `190000000` not `104000000`), the blanket refusal, the commitment-not-funded flag, and non-arms-length. Its `gold-replay` harness must reproduce those **12 attempted rows exactly** before the corpus pull runs (verified GREEN, offline, deterministic). The **5 deferred rows** are **disclosed and reason-tagged** in the replay output, **never silently skipped and never fed to the model**:
    - the **3 takeout rows** — `takeout_channel`/`takeout_lender` is debt-COUPON inference deferred to v2; v1 still recovers the loan amount + LTV (debt *quantity*) on these where single-parcel;
    - the **1 `still_outstanding`** pari-passu split note — no clean single amount in the public `document_amt`;
    - the **1 `coop_unit_deed`** (2420 Morris Ave) — **a soft gold label, not a v1 extraction gap**: this parcel records in public ACRIS as `CR`/`F1` with family-trust grantors, i.e. reads as a non-arms-length family disposition. Both labels exclude the comp from the engine, so the model input is unchanged either way.
  - **Grounding audit (pre-corpus discipline):** the replay also reports, for each attempted row, whether the public-record read is strongly **grounded** or a **soft** label — surfacing which fixtures are publicly grounded vs. carry hand-knowledge, even where the row passes. (Finding: the gold's arms-length deeds are not all `AP` — 300 W 44th and 1905 Third record as `CR` — so the `AP`-only corpus filter is narrower than the gold; the gold set was not row-by-row public-record-verified, hence the audit.)
